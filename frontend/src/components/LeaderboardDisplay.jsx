...
function LeaderboardDisplay({ leaderboard }) {
    ...
    // Get current page data and calculate total pages
    ...
    // Separate top 3 players from others for different rendering
    const topThreeOnPage = currentPlayersOnPage.filter(p => p.rank <= 3);
    const othersOnPage = currentPlayersOnPage.filter(p => p.rank > 3);

    // Helper function to get rank-based icons
    const getRankIcon = (rank) => {
        if (rank <= 3) return <span className="text-2xl">👑</span>;
        return <span className="text-xl">🏆</span>;
    };

    // Render top 3 podium cards with different styling for gold, silver, bronze
    const getTopThreeCard = (user, position) => {
        // Configuration for each podium position (0=gold, 1=silver, 2=bronze)
        const configs = [
            {
                // Gold - First Place (center position, tallest)
            ...
            {
                // Silver - Second Place (left position, medium height)
                ...
                order: 'order-1', // Left position in flexbox
                marginTop: 'mt-4 sm:mt-8'
            },
            {
                // Bronze - Third Place (right position, medium height)
                ...
                avatarSize: 'w-12 h-12 sm:w-16 sm:h-16',
                avatarIcon: 'text-2xl sm:text-3xl',
                order: 'order-3', // Right position in flexbox
                marginTop: 'mt-4 sm:mt-8'
            }
        ];
    ...
    return (
        <div className="bg-white/95 backdrop-blur rounded-t-3xl shadow-2xl min-h-96 border-t-4 border-orange-400">
            <div className="p-6">
                ...
                {/* Champions Podium Section - Only show if top 3 players are on current page */}
                {topThreeOnPage.length > 0 && (
                    <div className="mb-8">
                        ...
                        {/* Grid with 3 columns to maintain podium layout on all screen sizes */}
